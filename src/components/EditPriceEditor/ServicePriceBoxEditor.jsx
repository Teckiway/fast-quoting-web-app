...
import SmallSpinnerComponent from '../smallspinner/spinnerComponent'
...
const ServicePriceBoxEditor = ({handleAlertPresentTrue, handleAlertMessage, handleAlertType})=> {
    ...
    const [dataFetched, setDatatFetched] = useState(false)
    ...
    useEffect(()=>{
        ...
        firestore.collection("ProductTemplates").doc(productId).get()
        .then(data=>{
            let resultingData = data.data()

            const {id, itemQuantity, productDescription, productName, tax, totalPrice , totaltaxPriceSum , typeOfBusiness , unitPrice, images, taxSelection,  } =  resultingData
...
console.log(images)
    ...
    return (
        <div className = 'service-box' >
            ...
            {
                dataFetched ? '' :  <SmallSpinnerComponent/> 
            }
        <div className = 'priceEditorDiv-flex' >
            <div className="productNameEditor">
            <input value={productName} onChange={(event)=>handleProductName(event.target.value)}  type="text" className= 'product-name-box' placeholder='Product / Services Name' /> 
            <textarea value={productDescription}  onChange={(event)=>handleProductDescription(event.target.value)} type="text" className= 'product-desc-box' placeholder='Product / Services Description'/> 
            ...
            <div className="productPricingEditor">
                ...
                <select value={taxSelection} onChange={(event)=>handleTaxSelect(event.target.value)}  className='options-select'>
                    <option value="No Tax" className='options-select'>No Tax</option>
                    <option value='10' className='options-select'>10% Tax</option>
                    </select>
                <select value={typeOfBusiness} onChange={(event)=>handleTypeOfBusiness(event.target.value)} className='options-select'>
                ...
                <button className='done-button' onClick={()=>handleOnDoneButton()} >Save</button>  
                {
